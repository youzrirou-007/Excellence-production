...
import { FormationSchemaViewer, FormationBlastStep, STEP_DELAYS_MS, STEP_COLORS } from './FormationSchemaViewer';
...
import { 
  ...
  Pause, Clock, Download, UserCheck, Shield
} from 'lucide-react';
...
export const FormationSlideDeck: React.FC<FormationSlideDeckProps> = ({ gabarit, onClose }) => {
  ...
  const [activeSimStep, setActiveSimStep] = useState<FormationBlastStep>('tous');
  const simTimerRef = useRef<NodeJS.Timeout | null>(null);

  // Certificate Modal State
  const [certificateModalOpen, setCertificateModalOpen] = useState(false);
  const [operatorName, setOperatorName] = useState('Agent Technique SMI');
  const [operatorRole, setOperatorRole] = useState('Boutefeu / Foreur Qualifié');
  const [certificateGenerating, setCertificateGenerating] = useState(false);

  const slide = slides[index];
  const totalSlides = slides.length;

  // Séquence ordonnée de détonation en millisecondes
  const BLAST_STEPS: FormationBlastStep[] = ['bouchon', 'g1', 'g2', 'g3', 'g4', 'radier_parements', 'voute', 'tous'];
  ...
  // Réinitialisation ou synchronisation du simulateur à chaque changement de slide
  useEffect(() => {
    if (slide.showSchema) {
      setActiveSimStep(slide.highlightStep || 'tous');
      setIsSimulating(false);
      if (simTimerRef.current) clearInterval(simTimerRef.current);
    }
  }, [index, slide.showSchema, slide.highlightStep]);

  // Boucle de simulation pas-à-pas
  useEffect(() => {
    if (isSimulating) {
      let currentIdx = BLAST_STEPS.indexOf(activeSimStep);
      if (currentIdx === -1 || currentIdx >= BLAST_STEPS.length - 1) {
        currentIdx = 0;
        setActiveSimStep(BLAST_STEPS[0]);
      }

      simTimerRef.current = setInterval(() => {
        currentIdx = (currentIdx + 1) % BLAST_STEPS.length;
        setActiveSimStep(BLAST_STEPS[currentIdx]);
      }, 1400);
    } else {
      if (simTimerRef.current) clearInterval(simTimerRef.current);
    }
    ...
    if (isCorrect) {
      ...
      try {
        confetti({
          ...
          spread: 70,
          origin: { y: 0.6 },
          colors: ['#f59e0b', '#ffd700', '#10b981', '#ffffff']
        });
      } catch (e) {
        // Ignorer silencieusement si canvas-confetti est restreint
      }
    }
  };
  ...
  const getStepLegendInfo = (step?: FormationBlastStep) => {
    switch (step) {
      case 'bouchon':
        return {
          ...
          name: 'Jaune Or (D0)',
          delay: '0 ms',
          description: "Bouchon central instantané pour créer la cavité de libération du massif",
        };
      case 'g1':
        return {
          ...
          name: 'Bleu (G1)',
          delay: '25 ms',
          description: "1er carré d'expansion pour première ouverture vers le vide central",
        };
      case 'g2':
        return {
          ...
          name: 'Rouge (G2)',
          delay: '50 ms',
          description: "2ème carré d'expansion (agrandissement de la cavité)",
        };
      case 'g3':
        return {
          ...
          name: 'Cyan (G3)',
          delay: '100 ms',
          description: "3ème carré d'expansion (dégrossissage intermédiaire)",
        };
      case 'g4':
        return {
          ...
          name: 'Orange (G4)',
          delay: '200 ms',
          description: "4ème carré (expansion périphérique externe)",
        };
      case 'radier_parements':
        return {
          colorClass: 'text-purple-400',
          dotBg: 'bg-purple-500',
          name: 'Violet (Sole & Parements)',
          delay: '350 - 500 ms',
          description: "Radier et parements latéraux (guidage du profil bas et parois)",
        };
      case 'voute':
        return {
          ...
          name: 'Or Clair (Voûte)',
          delay: '750 - 1000 ms',
          description: "Couronne et voûte finale (découpage régulier du toit)",
        };
      case 'tous':
      default:
        return {
          colorClass: 'text-amber-400',
          dotBg: 'bg-gradient-to-r from-amber-400 via-cyan-400 to-rose-400',
          name: 'Séquence Complète',
          delay: '0 à 1000 ms',
          description: "Vue d'ensemble de la séquence de tir complète par groupes de micro-retards",
        };
    }
  };

  // Téléchargement du Certificat Officiel PDF SMI
  const handleGenerateCertificate = () => {
    ...
    try {
      ...
      // Fond de certificat noble et bords dorés
      doc.setFillColor(252, 250, 242);
      doc.rect(0, 0, width, height, 'F');

      // Double cadre doré d'Excellence
      ...
      doc.triangle(width - 12, 12, width - 12 - cornerSize, 12, width - 12, 12 + cornerSize, 'F');
      doc.triangle(12, height - 12, 12 + cornerSize, height - 12, 12, height - 12 - cornerSize, 'F');
      doc.triangle(width - 12, height - 12, width - 12 - cornerSize, height - 12, width - 12, height - 12 - cornerSize, 'F');

      // Logo SMI
      try {
        doc.addImage(excellenceLogoImg, 'PNG', width / 2 - 14, 18, 28, 28);
      } catch (e) {
        // image fallback
      }
      ...
      doc.text("SOCIÉTÉ MÉTALLURGIQUE D'IMITER (SMI)", width / 2, 52, { align: 'center' });

      doc.setFontSize(9);
      doc.setTextColor(100, 116, 139);
      doc.text("CENTRE DE FORMATION & D'EXCELLENCE TECHNIQUE MINIÈRE", width / 2, 57, { align: 'center' });
      ...
      doc.text("CERTIFICAT D'EXCELLENCE MINIÈRE", width / 2, 74, { align: 'center' });

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(11);
      doc.setTextColor(71, 85, 105);
      doc.text("Le jury technique atteste que :", width / 2, 85, { align: 'center' });

      // Nom du Récipiendaire en Grand
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(20);
      doc.setTextColor(180, 120, 20);
      doc.text((operatorName || "AGENT TECHNIQUE SMI").toUpperCase(), width / 2, 98, { align: 'center' });

      // Titre / Rôle
      doc.setFont('helvetica', 'italic');
      doc.setFontSize(11);
      doc.setTextColor(71, 85, 105);
      doc.text(operatorRole || "Opérateur / Boutefeu Spécialisé", width / 2, 105, { align: 'center' });
      ...
      const moduleName = allSlides[0]?.title || `Standard Gabarit ${gabarit.toUpperCase()}`;
      
      const validationText = `A validé avec succès l'évaluation théorique et opérationnelle des règles de l'art du module :`;
      doc.text(validationText, width / 2, 118, { align: 'center' });

      doc.setFont('helvetica', 'bold');
      doc.setFontSize(13);
      doc.setTextColor(15, 23, 42);
      doc.text(`« ${moduleName} »`, width / 2, 126, { align: 'center' });

      // Métadonnées & Identifiant unique
      const certDate = new Date().toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' });
      const certId = `SMI-EXC-${gabarit.toUpperCase().replace(/[^A-Z0-9]/g, '')}-${Date.now().toString().slice(-6)}`;

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.setTextColor(100, 116, 139);
      doc.text(`Délivré à Imiter le ${certDate}`, 24, 155);
      doc.text(`Identifiant d'homologation : ${certId}`, 24, 160);
      doc.text(`Mention : Maîtrise Opérationnelle & Sécurité Confirmée`, 24, 165);

      // Tampon Doré Sceau d'Excellence
      doc.setDrawColor(212, 175, 55);
      doc.setLineWidth(1);
      doc.circle(width / 2, 160, 14);
      doc.setFontSize(6.5);
      doc.setTextColor(180, 140, 40);
      doc.setFont('helvetica', 'bold');
      doc.text("EXCELLENCE SMI", width / 2, 158, { align: 'center' });
      doc.text("DIRECTION TECHNIQUE", width / 2, 162, { align: 'center' });
      doc.text("★ VALIDÉ ★", width / 2, 166, { align: 'center' });

      // Signatures
      doc.setFontSize(9);
      doc.setTextColor(15, 23, 42);
      doc.text("Le Responsable Formation", width - 75, 155, { align: 'center' });
      doc.text("La Direction Technique Minière", width - 75, 170, { align: 'center' });
      doc.setFont('helvetica', 'italic');
      doc.setFontSize(8);
      doc.setTextColor(148, 163, 184);
      doc.text("[ Cachet Électronique & Homologation ]", width - 75, 176, { align: 'center' });

      doc.save(`Certificat_Excellence_SMI_${operatorName.replace(/\s+/g, '_')}_${gabarit}.pdf`);
  ...
  const handleDownloadSynthesis = () => {
    ...
    const addWrappedText = (text: string, fontSize: number, fontStyle: 'normal' | 'bold' = 'normal', color: [number, number, number] = [30, 41, 59]) => {
    ...
    try {
      const logoWidth = 22;
      const logoHeight = 22;
      doc.addImage(excellenceLogoImg, 'PNG', (pageWidth - logoWidth) / 2, y, logoWidth, logoHeight);
      y += logoHeight + 4;
    } catch (e) {}
    ...
    doc.setFontSize(9);
    doc.setTextColor(180, 140, 40);
    doc.text('EXCELLENCE · OPÉRATIONS · INNOVATION · SMI IMITER', pageWidth / 2, y, { align: 'center' });
    y += 8;
    ...
    addWrappedText(`FICHE MÉMOTECHNIQUE DE TERRAIN`, 16, 'bold', [15, 23, 42]);
    addWrappedText(moduleTitle, 12, 'bold', [180, 140, 40]);
    y += 4;

    allSlides.forEach((s, idx) => {
      if (s.type === 'sommaire') return;
      ...
      addWrappedText(`[${idx + 1}] ${s.kicker || ''}`, 8, 'bold', [180, 140, 40]);
      addWrappedText(s.title, 12, 'bold', [15, 23, 42]);
      y += 1;

      if (s.body?.length) {
        s.body.forEach(p => addWrappedText(p, 10, 'normal', [51, 65, 85]));
        y += 2;
      }

      if (s.metrics?.length) {
        addWrappedText('DONNÉES CLÉS :', 9, 'bold', [15, 23, 42]);
        s.metrics.forEach(m => {
          const line = `• ${m.label} : ${m.value}${m.unit ? ' ' + m.unit : ''}${m.subtext ? ' (' + m.subtext + ')' : ''}`;
          addWrappedText(line, 9.5, 'normal', [51, 65, 85]);
        });
        y += 2;
      }

      if (s.ruleOfArt) {
        addWrappedText(`RÈGLE DE L'ART : ${s.ruleOfArt}`, 9.5, 'normal', [15, 100, 60]);
        y += 2;
      }

      if (s.safety) {
        addWrappedText(`⚠ SÉCURITÉ : ${s.safety}`, 9.5, 'bold', [180, 40, 40]);
        y += 2;
      }

      if (s.quiz) {
        addWrappedText(`QUIZ : ${s.quiz.question}`, 9.5, 'bold', [15, 23, 42]);
        s.quiz.options.forEach((opt, i) => {
          addWrappedText(`  ${i === s.quiz!.correctAnswer ? '✓' : '–'} ${opt}`, 9, 'normal', [51, 65, 85]);
        });
        addWrappedText(`Explication : ${s.quiz.explanation}`, 9, 'normal', [51, 65, 85]);
        y += 2;
      }
    ...
    doc.text(`Document généré depuis la plateforme EXCELLENCE — ${new Date().toLocaleDateString('fr-FR')}`, pageWidth / 2, y, { align: 'center' });

    doc.save(`EXCELLENCE_Memo_${gabarit}.pdf`);
  ...
  }, [index, totalSlides, onClose, certificateModalOpen]);
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-gradient-to-b from-white via-amber-50/30 to-white flex flex-col justify-between overflow-hidden font-sans select-none">
      ...
      <div className="border-b border-amber-200 px-4 sm:px-6 py-3 flex items-center justify-between shadow-md relative z-20 overflow-hidden text-white bg-slate-900">
        <div 
        ...
        <div className="flex items-center gap-3 sm:gap-4 relative z-10">
          <div className="flex items-center gap-2 bg-slate-950 p-1.5 px-3 rounded-xl border border-amber-500/40">
            ...
            />
            <div className="hidden md:block text-left border-l border-slate-800 pl-2">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="hidden sm:inline-flex items-center gap-1 text-[10px] font-mono text-slate-300 bg-slate-950/60 px-2 py-0.5 rounded border border-slate-800">
                <Clock className="w-3 h-3 text-amber-400" /> ~4 min
              </span>
            </div>
            <h3 className="text-white text-sm md:text-base font-black tracking-tight truncate max-w-[200px] sm:max-w-xs md:max-w-md">
              {slide.title}
            </h3>
          </div>
        </div>

        {/* Center: Slide Progress Dots with Tooltip */}
        <div className="hidden lg:flex items-center gap-1.5 max-w-sm w-full mx-4">
          {slides.map((sItem, i) => (
            <button
              key={i}
              onClick={() => handleJumpToSlide(i)}
              className={`h-2 flex-1 rounded-full transition-all cursor-pointer relative group ${
                i === index 
              ...
              title={`${i + 1}. ${sItem.title}`}
            >
              <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 hidden group-hover:block z-30 bg-slate-900 text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg whitespace-nowrap border border-amber-400/40 pointer-events-none">
                {i + 1}. {sItem.title}
              </div>
            </button>
          ))}
        ...
        {/* Right Controls: Sommaire, Certificate, Fullscreen Toggle & Close */}
        <div className="flex items-center gap-1.5 sm:gap-2 relative z-10">
          <button
            onClick={() => handleJumpToSlide(1)}
            className="p-2 sm:px-3 sm:py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-amber-400 hover:text-white transition-all cursor-pointer border border-amber-500/30 flex items-center gap-1.5 text-xs font-bold"
            title="Sommaire Interactif"
          >
            <ListOrdered className="w-4 h-4" />
            <span className="hidden xl:inline">Sommaire</span>
          </button>

          <button
            onClick={() => setCertificateModalOpen(true)}
            className="px-3 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 text-slate-950 transition-all cursor-pointer shadow-md flex items-center gap-1.5 text-xs font-black"
            title="Générer mon Certificat Officiel SMI"
          >
          ...
          <button
            ...
            title={isFullscreen ? "Quitter le plein écran" : "Plein Écran Immersion Totale"}
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 flex items-center justify-center relative bg-gradient-to-b from-slate-50 via-white to-slate-50">
        ...
        {/* Maximize Card Container with smooth directional transition */}
        <div className="max-w-6xl w-full relative">
          <AnimatePresence mode="wait" custom={direction}>
            <motion.div
              key={index}
              custom={direction}
              initial={{ opacity: 0, x: direction > 0 ? 30 : -30, scale: 0.99 }}
              animate={{ opacity: 1, x: 0, scale: 1 }}
              exit={{ opacity: 0, x: direction > 0 ? -30 : 30, scale: 0.99 }}
              transition={{ duration: 0.28, ease: [0.25, 1, 0.5, 1] }}
              className="w-full bg-white border-2 border-slate-200 rounded-3xl shadow-2xl p-6 md:p-10 space-y-6 relative overflow-hidden"
            >
              {/* Top Gold Bar */}
              ...
              {/* SLIDE TYPE 1: INTRO SLIDE */}
              {slide.type === 'intro' ? (
                <div className="space-y-6">
                  <div className="relative rounded-2xl overflow-hidden border-2 border-amber-500/40 shadow-xl h-52 md:h-64 flex items-center justify-center text-center p-6">
                    <img 
                    ...
                    <div className="relative z-10 space-y-3 max-w-3xl">
                      <div className="inline-flex items-center gap-2 bg-slate-950/80 backdrop-blur-md text-amber-400 border border-amber-400/50 text-xs font-black px-4 py-1.5 rounded-full uppercase tracking-wider shadow-lg">
                        <Award className="w-4 h-4 text-amber-400" /> MODULE HAUTE PERFORMANCE SMI
                      </div>
                      <h1 className="gold-title my-1 select-none text-2xl md:text-4xl font-black uppercase tracking-tight drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)]">
                        {slide.title}
                      </h1>
                    </div>
                  </div>
                  ...
                  {/* Key Metrics - 80% CONTRAST MAINTAINED */}
                  {slide.metrics && (
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                      {slide.metrics.map((m, mIdx) => (
                        <div key={mIdx} className="p-4 rounded-2xl shadow-xl border-2 border-amber-500/40 relative overflow-hidden flex flex-col justify-between text-center group transition-all bg-slate-950">
                          <div 
                          ...
                          />
                          {/* 20% subtle dark tint overlay = 80% visual contrast preserved */}
                          <div className="absolute inset-0 bg-slate-950/20 pointer-events-none" />
                          <div className="relative z-10 space-y-1.5">
                            <span className="text-[11px] font-black uppercase tracking-wider text-amber-300 block drop-shadow-[0_2px_4px_rgba(0,0,0,1)]">{m.label}</span>
                            <div className="text-2xl sm:text-3xl font-black text-white tracking-tight drop-shadow-[0_3px_8px_rgba(0,0,0,1)]">
                              {m.value} <span className="text-xs font-black text-amber-300 drop-shadow-[0_2px_4px_rgba(0,0,0,1)]">{m.unit}</span>
                            </div>
                            {m.subtext && <span className="text-[10px] text-slate-100 font-bold block drop-shadow-[0_2px_4px_rgba(0,0,0,1)]">{m.subtext}</span>}
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                /* SLIDE TYPE 2: INTERACTIVE SOMMAIRE */
                <div className="space-y-6">
                  <div className="p-5 rounded-2xl border border-amber-500/40 shadow-lg relative overflow-hidden flex items-center gap-4">
                    <div 
                    ...
                    <div className="relative z-10">
                      <h4 className="font-black text-lg text-amber-300 drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">Sommaire Interactif de la Formation</h4>
                      <p className="text-xs text-slate-200 drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">Cliquez sur n'importe quel chapitre pour y accéder directement.</p>
                    </div>
                  </div>
                ...
                /* SLIDE TYPE 3: QUIZ AVEC RÉCOMPENSE & CERTIFICAT */
                <div className="space-y-6">
                  <div className="flex items-center justify-between gap-3 text-amber-600 bg-amber-50 p-4 rounded-2xl border border-amber-200">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-600">Validez vos compétences pour débloquer votre Certificat d'Excellence.</p>
                      </div>
                    </div>
                    {quizSubmitted && (
                      <button
                        onClick={() => setCertificateModalOpen(true)}
                        className="px-4 py-2 bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 text-slate-950 font-black text-xs uppercase tracking-wider rounded-xl flex items-center gap-1.5 shadow-md cursor-pointer transition-all"
                      >
                        <Award className="w-4 h-4" /> Certificat PDF
                      </button>
                    )}
                  </div>
                  ...
                  {slide.quiz && (
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6 space-y-4">
                      ...
                      {!quizSubmitted ? (
                        ...
                        <div className="p-4.5 rounded-2xl border border-amber-500/30 relative overflow-hidden shadow-md">
                          <div 
                            className="absolute inset-0 bg-cover bg-center bg-no-repeat pointer-events-none"
                            style={{ backgroundImage: `url(${carteKpisImg})`, filter: 'contrast(1.15) brightness(0.95)' }}
                          />
                          ...
                          <div className="relative z-10 space-y-1.5 text-xs">
                            <p className="font-black flex items-center gap-2 text-amber-300 drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">
                              <Sparkles className="w-4 h-4 text-amber-400" /> Explication de l'Expert :
                            </p>
                            <p className="text-slate-100 font-medium leading-relaxed drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">{slide.quiz.explanation}</p>
                          </div>
                        </div>
                      )}
                    </div>
                ...
                /* SLIDE TYPE 4: SYNTHESIS */
                <div className="space-y-6">
                  <div className="p-6 rounded-2xl shadow-lg border border-amber-500/40 relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div 
                      className="absolute inset-0 bg-cover bg-center bg-no-repeat pointer-events-none"
                      style={{ backgroundImage: `url(${carteKpisImg})`, filter: 'contrast(1.15) brightness(0.95)' }}
                    />
                    ...
                    <div className="relative z-10 flex items-center gap-4">
                      <div className="bg-slate-900/80 backdrop-blur-md p-2.5 rounded-xl border border-amber-400/40 flex-shrink-0 shadow-md">
                        <Award className="w-8 h-8 text-amber-400" />
                      </div>
                      <div>
                        <h4 className="font-black text-xl text-amber-300 drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">Fiche Mémotechnique & Certificat</h4>
                        <p className="text-xs text-slate-200 drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">Synthèse opérationnelle pour le boutefeu et chef de chantier.</p>
                      </div>
                    </div>
                    ...
                    <div className="relative z-10 flex items-center gap-2 w-full sm:w-auto">
                      <button
                        ...
                        className="flex-1 sm:flex-none px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-amber-400 border border-amber-400/40 font-black text-xs uppercase tracking-wider rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer shadow-md"
                      >
                        {copied ? <Check className="w-4 h-4" /> : <Download className="w-4 h-4" />}
                        {copied ? 'Téléchargé !' : 'Fiche Synthèse'}
                      </button>

                      <button
                        onClick={() => setCertificateModalOpen(true)}
                        className="flex-1 sm:flex-none px-4 py-2.5 bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 text-slate-950 font-black text-xs uppercase tracking-wider rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer shadow-md"
                      >
                        <Award className="w-4 h-4" /> Certificat PDF
                      </button>
                    </div>
                ...
                /* SLIDE TYPE 5: TECHNICAL SLIDE WITH SCHEMAS & SIMULATION */
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                  <div className={`space-y-5 ${slide.showSchema ? 'lg:col-span-7' : 'lg:col-span-12'}`}>
                    ...
                    {/* Key Metrics - 80% CONTRAST */}
                    {slide.metrics && (
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-1">
                        {slide.metrics.map((m, mIdx) => (
                          <div key={mIdx} className="p-3 rounded-2xl shadow-xl border-2 border-amber-500/40 relative overflow-hidden text-center space-y-1 group transition-all bg-slate-950">
                            <div 
                              className="absolute inset-0 bg-cover bg-center bg-no-repeat pointer-events-none transition-transform duration-500 group-hover:scale-105"
                              style={{ backgroundImage: `url(${carteKpisImg})`, filter: 'contrast(1.15) brightness(0.95)' }}
                            />
                            {/* 20% tint overlay = 80% marble contrast preserved */}
                            <div className="absolute inset-0 bg-slate-950/20 pointer-events-none" />
                            <div className="relative z-10">
                              <span className="text-[10px] font-black uppercase tracking-wider text-amber-300 block drop-shadow-[0_2px_4px_rgba(0,0,0,1)]">{m.label}</span>
                              <div className="text-xl sm:text-2xl font-black text-white drop-shadow-[0_3px_6px_rgba(0,0,0,1)]">
                                {m.value} <span className="text-xs font-black text-amber-300 drop-shadow-[0_2px_4px_rgba(0,0,0,1)]">{m.unit}</span>
                              </div>
                              {m.subtext && <span className="text-[9px] text-slate-100 font-bold block truncate drop-shadow-[0_2px_4px_rgba(0,0,0,1)]">{m.subtext}</span>}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  {/* Schema Graphic Column WITH SIMULATOR & STEPPER */}
                  {slide.showSchema && (
                    <div className="lg:col-span-5 bg-slate-950 rounded-2xl p-4 sm:p-5 border-2 border-slate-800 shadow-2xl space-y-3.5">
                      <div className="flex items-center justify-between text-white border-b border-slate-800 pb-2.5">
                        <span className="text-[10px] font-black uppercase text-amber-400 tracking-wider flex items-center gap-1.5">
                          <Eye className="w-3.5 h-3.5" /> Séquence de Tir Interactive
                        </span>
                        ...
                        >
                          ...
                          {isSimulating ? 'Pause' : 'Simuler Tir'}
                        </button>
                      </div>

                      {/* SVG Schema Viewer with Hover Inspector */}
                      <div className="h-60 sm:h-64 w-full flex items-center justify-center relative">
                        <FormationSchemaViewer
                          gabarit={slide.schemaType || (gabarit as GabaritType) || '12m2'}
                          highlightStep={activeSimStep}
                          className="w-full h-full max-h-60"
                        />
                      </div>

                      {/* Micro-delays Step Selector Bar */}
                      <div className="grid grid-cols-4 gap-1.5 pt-1">
                        {(['bouchon', 'g1', 'g2', 'g3', 'g4', 'radier_parements', 'voute', 'tous'] as FormationBlastStep[]).map((st) => {
                          const isCur = activeSimStep === st;
                          const labels: Record<FormationBlastStep, string> = {
                            bouchon: 'D0 0ms',
                            g1: 'G1 25ms',
                            g2: 'G2 50ms',
                            g3: 'G3 100ms',
                            g4: 'G4 200ms',
                            radier_parements: 'Sole',
                            voute: 'Voûte',
                            tous: 'Tous'
                          };
                          return (
                      ...
                      {/* Dynamic Legend */}
                      {(() => {
                        const legend = getStepLegendInfo(activeSimStep);
                        return (
                          <div className="bg-slate-900/90 p-3 rounded-xl border border-slate-800 text-center space-y-1">
                            <div className="flex items-center justify-center gap-2 text-xs">
                              <span className={`w-3 h-3 rounded-full ${legend.dotBg} inline-block flex-shrink-0 shadow-sm`} />
                              <span className={`font-black ${legend.colorClass} uppercase tracking-wider`}>
                                {legend.name} • {legend.delay}
                              </span>
                            </div>
      ...
      {/* Modal Certificat Officiel d'Excellence SMI */}
      {certificateModalOpen && (
        <div className="fixed inset-0 z-[10000] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border-2 border-amber-500/50 rounded-3xl p-6 sm:p-8 max-w-lg w-full text-white shadow-2xl space-y-5 relative">
            <button
            ...
            <div className="flex items-center gap-3 text-amber-400">
              <div className="p-2.5 bg-amber-500/20 rounded-2xl border border-amber-400/40">
                <Award className="w-7 h-7 text-amber-400" />
              </div>
              <div>
                <h3 className="text-lg font-black tracking-tight text-white">Certificat Officiel d'Excellence</h3>
                <p className="text-xs text-slate-400">Société Métallurgique d'Imiter (SMI)</p>
              </div>
            </div>

            <p className="text-xs text-slate-300 leading-relaxed">
              Personnalisez votre certificat d'accomplissement officiel avant de générer le document PDF haute résolution.
            </p>

            <div className="space-y-4 text-xs font-bold">
              <div>
                <label className="text-amber-300 block mb-1.5 uppercase tracking-wider text-[10px]">
                ...
                <input
                  ...
                  placeholder="Ex: Mohamed Amine - Boutefeu Expert"
                  className="w-full bg-slate-950 border border-slate-700 focus:border-amber-400 rounded-xl px-4 py-2.5 text-white font-medium outline-none transition-colors"
                />
              </div>

              <div>
                <label className="text-amber-300 block mb-1.5 uppercase tracking-wider text-[10px]">
                  Fonction / Titre Technique :
                </label>
                <input
                  type="text"
                  value={operatorRole}
                  onChange={(e) => setOperatorRole(e.target.value)}
                  placeholder="Ex: Boutefeu Qualifié / Chef d'Équipe"
                  className="w-full bg-slate-950 border border-slate-700 focus:border-amber-400 rounded-xl px-4 py-2.5 text-white font-medium outline-none transition-colors"
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <button
                ...
                disabled={certificateGenerating || !operatorName.trim()}
                className="flex-1 py-2.5 bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 disabled:opacity-40 text-slate-950 font-black rounded-xl text-xs uppercase tracking-wider transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer"
              >
                <Download className="w-4 h-4" />
                {certificateGenerating ? 'Génération...' : 'Télécharger PDF'}
              </button>
            </div>
          </div>
        </div>
