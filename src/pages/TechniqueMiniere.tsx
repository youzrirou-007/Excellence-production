...
import { SectionHubModal } from './TechniqueMiniere/SectionHubModal';
...
export const TechniqueMiniere: React.FC = () => {
  // Navigation State: 'home' (4 sections), 'hub' (carte des 7 sous-modules de la section), 'detail' (vue onglets)
  const [viewMode, setViewMode] = useState<'home' | 'hub' | 'detail'>('home');
  const [activeTab, setActiveTab] = useState<TabType>('schema');
  const [gabarit, setGabarit] = useState<GabaritType>('12m2');

  // Quand on clique sur une grande carte de galerie -> ouverture directe du Hub de sélection de module
  const handleSelectGabarit = (selected: GabaritType) => {
    setGabarit(selected);
    setViewMode('hub');
  };

  // Quand on clique sur une sous-carte du Hub -> redirection immédiate vers l'onglet choisi
  const handleSelectTabFromHub = (tab: TabType) => {
    setActiveTab(tab);
    setViewMode('detail');
  };
  ...
  return (
    <div className="bg-white min-h-screen">
      <AnimatePresence mode="wait">
        ...
        {/* VUE 1 : GRILLES DES 4 GRANDES SECTIONS DE GALERIES */}
        {viewMode === 'home' && (
          <motion.div
            key="home-cards-view"
            initial={{ opacity: 0 }}
        ...
        {/* VUE 2 : HUB DE SOUS-CARTES POUR LA SECTION CHOISIE (Plan de tir, 3D, Guide Forage, Explosifs, Bourrage, Rentabilité, Académie) */}
        {viewMode === 'hub' && (
          <motion.div
            key="hub-modal-view"
          ...
          >
            <SectionHubModal
              gabarit={gabarit}
              onSelectTab={handleSelectTabFromHub}
              onBack={handleBackToHome}
            />
          </motion.div>
        )}

        {/* VUE 3 : DOSSIER COMPLET AVEC TOUS LES ONGLETS ACTIFS ACCESSIBLES */}
        {viewMode === 'detail' && (
          <motion.div
            ...
            <div className="p-6 rounded-3xl border border-amber-500/30 shadow-xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="relative z-10 flex items-center gap-3.5 mx-auto md:mx-0 text-center md:text-left">
                <button
                  ...
                  title="Retour au menu des modules de la section"
                >
                  <ChevronLeft className="w-5 h-5 transition-transform group-hover:-translate-x-1" />
                </button>
                <div className="text-center md:text-left">
                  <span className="text-[10px] font-black uppercase text-amber-200 tracking-widest block">
                    Galerie active : {gabarit === '9m2' ? '9 m² — Traçage SMI' : gabarit === '9m2_intl' ? '9 m² — Traçage International' : gabarit === '12m2_intl' ? '12 m² — Standard International' : '12 m² — Gabarit SMI'}
                  </span>
              ...
              {/* Accès Badge + Bouton Hub */}
              <div className="relative z-10 mx-auto md:mx-0 flex items-center gap-2">
                <button
                  onClick={handleBackToHub}
                  className="inline-flex items-center gap-1.5 bg-slate-900/90 hover:bg-slate-950 text-amber-300 hover:text-white text-[10px] font-black uppercase tracking-wider px-3 py-2 rounded-xl border border-amber-400/40 shadow-lg cursor-pointer transition-all"
                  title="Voir la grille des modules"
                >
